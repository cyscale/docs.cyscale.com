...
#### Google Cloud organization support

If you want to onboard a larger Google Cloud estate, you can additionally enable organization support in step 1.

With organization support enabled, you still choose a project in which the Cyscale service account will be created, but you also provide the Google Cloud **organization ID** and optionally exclude specific project IDs. Cyscale will then automatically add the rest of the projects covered by that organization-level configuration.

For a complete organization onboarding workflow, see [Organization Onboarding](./organization-onboarding.mdx).
...
When organization support is enabled, the generated Terraform flow also accepts the organization ID so the service account can be granted the required access at the organization level.
...
During onboarding, you can also configure connector notifications and, for cloud connectors, sync exclusions for specific asset types or key:value tag pairs.
...
- **Vertex AI Viewer** - If AI Security is enabled and you want Cyscale to sync Google Cloud AI assets, grant `roles/aiplatform.viewer` or an equivalent custom role with the AI Platform read and list permissions shown in the connector permission update flow.
...
Recent Google Cloud coverage additions include:

- Cloud Run services and revisions, together with their relationships to Secret Manager, Cloud SQL, load balancers, and other supporting services
- Cloud Functions 2nd Gen
- Serverless VPC Access connectors
- Artifact Registry repositories and vulnerability context from Artifact Analysis / OS scanning
- Container Registry (GCR) usage visibility
- Secret Manager relationships
- Vertex AI models, endpoints, datasets, feature stores, and related AI Security context when the AI Security module and permissions are enabled

This makes the Google Cloud connector more useful both for service coverage and for contextual prioritization across serverless, storage, IAM, and vulnerability workflows.
