...
The list of Google Cloud resources that Cyscale can handle are listed in the tables below, along with the number of controls that check their configuration
...
In addition, Cyscale offers 20 more Google Cloud controls that are not specific to a service or asset type. These are called cloud-wide controls.
